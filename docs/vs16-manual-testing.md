...
- `backend/dev/seed/vs16-seed.sql`: creates its own venue, event and pending booking.
...
1. Select Venue Staff in the role selector.
...
Direct route after selecting Venue Staff:
...
and detail route. No Organiser `event_requests` row is seeded. Backend authorization
remains deferred; the role selector is not authenticated API access control.
...
wrapped long requirements. Check desktop scrolling and narrow viewport layout.
...
Repeated cleanup is a harmless no-op when all three demo records are absent.
